pipeline {
    ...
    stages {
        stage("Continuous Integration / Intégration Continue") {
            steps {
                git branch: 'main', url: 'https://github.com/RJPinoy/vpbot-backend.git'
                sh 'composer install'
            }
        }
        stage("Continuous Delivery / Livraison Continue") {
            steps {
                sh "docker build . -t ${DOCKERHUB_USERNAME}/vpbot-backend"
                ...
                sh "docker push ${DOCKERHUB_USERNAME}/vpbot-backend"
            }
        }
    }
}